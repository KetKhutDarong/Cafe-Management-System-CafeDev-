...
import { Mail, Lock, Eye, EyeOff, ShieldCheck, AlertCircle, Coffee, ArrowRight, Loader2 } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "motion/react";
import { login as loginService, getGoogleAuthUrl } from "@/services/authService";
...
export default function LoginPage() {
      ...
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        ...
        if (["admin", "manager"].includes(userData.role)) {
          ...
          if (hasAdminPerms && perms.manageOrders === false) {
            navigate("/admin");
          } else if (perms.manageOrders === true) {
            navigate("/staff");
          } else {
            // If they have NO special permissions, send them to profile
            navigate("/profile");
          }
        }
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.error || t('auth.invalidCredentials');
      setError(errMsg);
      toast.error(errMsg);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 sm:p-6 font-sans transition-colors duration-300">
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 bg-card rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] overflow-hidden lg:min-h-[700px] border border-border transition-colors duration-300 relative">
        ...
        <div className="hidden lg:flex flex-col relative bg-[#0a0a0a] p-16 justify-between overflow-hidden">
          <div className="absolute inset-0 z-0">
             <img 
              src="https://images.unsplash.com/photo-1509042239860-f550ce710b93?q=80&w=2000&auto=format&fit=crop" 
              alt="Artisan Coffee" 
              className="w-full h-full object-cover opacity-60 scale-105 hover:scale-100 transition-transform duration-[10s] ease-out"
          ...
          <div className="relative z-10">
            <Link to="/" className="flex items-center gap-4 group">
              <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center group-hover:rotate-[-12deg] transition-all duration-500 shadow-2xl shadow-primary/30 overflow-hidden">
                <img src="/cafedev_logo.png" alt="Logo" className="w-10 h-10 object-contain" fallback-icon="Coffee" />
              </div>
          ...
          <div className="relative z-10">
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full border border-white/20 mb-6">
                ...
                <span className="text-[10px] font-black tracking-widest text-white uppercase">{t('auth.secureLogin')}</span>
              </div>
          ...
          >
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">{t('auth.email')}</label>
                </div>
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-muted/30 border border-border rounded-2xl font-semibold text-sm focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-card-foreground placeholder:text-muted-foreground/50"
                    required
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" className="text-[10px] font-black text-primary uppercase tracking-widest hover:underline">{t('auth.forgotPassword')}</button>
                </div>
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-12 pr-12 py-4 bg-muted/30 border border-border rounded-2xl font-semibold text-sm focus:ring-2 focus:ring-primary focus:border-transparent transition-all text-card-foreground placeholder:text-muted-foreground/50"
                    required
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-bold text-sm flex items-center justify-center gap-3 hover:opacity-90 active:scale-[0.98] transition-all shadow-xl shadow-primary/20 group"
              >
                {isLoading ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className="flex items-center justify-center gap-3 py-3.5 border border-border rounded-2xl hover:bg-muted font-bold text-[11px] uppercase tracking-widest text-card-foreground transition-all hover:border-muted-foreground/20"
              >
              ...
              <button className="flex items-center justify-center gap-3 py-3.5 border border-border rounded-2xl hover:bg-muted font-bold text-[11px] uppercase tracking-widest text-card-foreground transition-all hover:border-muted-foreground/20">
                <AppleIcon />
                Apple
              </button>
            </div>
